...
export function WhiteboardOnboarding({ onDismiss }: WhiteboardOnboardingProps) {
	...
	const measureAnchors = useCallback(() => {
		...
		const penButton = document.querySelector(
			'[aria-label="Pen"]',
		) as HTMLElement | null;
		const lassoButton = document.querySelector(
			'[aria-label="Lasso Solve"]',
		) as HTMLElement | null;
	...
	return (
		...
		>
			...
			<div
				data-hint
				className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-center pointer-events-none opacity-0"
			>
			...
			<div
				data-hint
				className="absolute flex flex-col items-center pointer-events-none opacity-0 "
				style={toolHintStyle}
			>
				<ArrowUp className="text-gray-300" />
				<p className="text-gray-400 text-sm text-center mt-1 translate-x-6">
					Pick a tool &amp;
					<br />
					start drawing
				</p>
			...
			<div
				data-hint
				className="absolute flex flex-col items-center pointer-events-none opacity-0 p-3 "
				style={lassoHintStyle}
			>
				<ArrowUp className="text-gray-300" />
				<p className="text-gray-400 text-sm text-center mt-1 -translate-x-6">
					<span className="text-gray-500 font-medium">Lasso Solve</span>
					<br />
					Circle a problem
					<br />
					for AI help
			...
			<div
				data-hint
				className="absolute flex flex-col items-start pointer-events-none opacity-0"
				style={backHintStyle}
			>
				<ArrowUp className="text-gray-300" />
