	...
	const fetchData = useCallback(async () => {
		try {
			const res = await fetch(
				`/api/teacher/concept-mastery?assignmentId=${assignmentId}`,
			);
			if (res.ok) {
			...
			<CardContent className="space-y-6">
				<div className="grid grid-cols-2 md:grid-cols-4 gap-4">
					...
					<div className="text-center p-3 bg-blue-50 rounded-lg">
						<div className="text-2xl font-bold text-blue-600 ">
							{modeBreakdown.feedback || 0}
						</div>
						<div className="text-xs text-muted-foreground">Feedback</div>
					</div>
					<div className="text-center p-3 bg-amber-50 rounded-lg">
						<div className="text-2xl font-bold text-amber-600 ">
							{modeBreakdown.suggest || 0}
						</div>
						<div className="text-xs text-muted-foreground">Suggestions</div>
					</div>
					<div className="text-center p-3 bg-green-50 rounded-lg">
						<div className="text-2xl font-bold text-green-600 ">
							{modeBreakdown.answer || 0}
						</div>
				...
				{concepts.length === 0 ? (
					<div className="text-center py-8 text-muted-foreground">
						...
						<p className="text-sm">
							Concept mastery will appear as students use AI help
						</p>
					</div>
				) : (
							...
							<div key={concept.concept} className="border rounded-lg p-3">
								<div className="flex items-center justify-between mb-2">
									<div className="flex items-center gap-2">
										<div
											className={`w-3 h-3 rounded-full ${getMasteryColor(concept.strugglingPercent)}`}
										/>
								...
								<div className="h-4 bg-muted rounded-full overflow-hidden flex mb-2">
									{concept.struggling > 0 && (
										<div
											...
											style={{
												width: `${pct(concept.struggling, concept.totalStudents)}%`,
											}}
								...
								{concept.strugglingPercent >= 50 && (
									<div className="mt-2 p-2 bg-red-50 border border-red-200 rounded text-xs text-red-700 flex items-center gap-2">
										...
										<span>
											{concept.strugglingPercent}% of students need help with
											this concept. Consider reviewing in class.
										</span>
									</div>
								)}
							</div>
