-- ============================================
-- COMPLETE FIX V2: RLS Policies + Metadata Sync
-- ============================================
...
-- Drop ALL profile policies (including any that may have been created previously)
...
-- Drop ALL whiteboard policies
DROP POLICY IF EXISTS "Teachers can view all whiteboards" ON whiteboards;
DROP POLICY IF EXISTS "Admins can view all whiteboards" ON whiteboards;
DROP POLICY IF EXISTS "Admins can update any whiteboard" ON whiteboards;
DROP POLICY IF EXISTS "Admins can delete any whiteboard" ON whiteboards;
...
-- ============================================
-- PART 7: ADD POLICIES FOR OTHER TABLES
-- ============================================
...
-- Drop existing policies for ai_usage if table exists
DO $$
BEGIN
  IF EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'ai_usage') THEN
    DROP POLICY IF EXISTS "Admins can view all ai_usage" ON ai_usage;

    -- Admins can view all AI usage
    EXECUTE 'CREATE POLICY "Admins can view all ai_usage" ON ai_usage
      FOR SELECT
      USING (
        (auth.jwt() -> ''user_metadata'' ->> ''role'') = ''admin''
      )';
...
WHERE schemaname = 'public'
  AND tablename IN ('profiles', 'whiteboards', 'classes', 'assignments', 'ai_usage')
...
-- COMPLETION MESSAGE
-- ============================================
SELECT '✅ Complete fix V2 applied! All policies updated and metadata synced.' as status;
